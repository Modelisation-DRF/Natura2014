...
Lecture_arbres <- function(file_arbre, ht, vol, climat) {
  ...
  if (length(nom_base_manquant) > 0) {
    arbres <- paste("Nom des variables de base incorrect dans le fichier des arbres: ", paste(nom_base_manquant, collapse = ", "))
  } else {
    # vérification de la ht si elle est fournie dans le fichier d'inventaire
    if (isFALSE(ht) && length(setdiff(nom_ht, nom)) > 0) {
      arbres <- paste0("Nom de la variable de hauteur incorrect dans le fichier des arbres")
    }
    # vérification du volume s'il est fourni dans le fichier d'inventaire
    if (isFALSE(vol) && length(setdiff(nom_vol, nom)) > 0) {
      arbres <- paste0("Nom de la variable du volume incorrect dans le fichier des arbres")
    }
    # verification si climat fourni
    if (isFALSE(climat) && length(setdiff(nom_clim, nom)) > 0) {
      arbres <- paste0("Nom des variables climatiques incorrect dans le fichier des arbres")
    }
    # verification si climat non fourni
    if (isTRUE(climat) && length(setdiff(nom_coord, nom)) > 0) {
      arbres <- paste0("Nom des variables de coordonnees incorrect dans le fichier des arbres")
    }
  }
