...
CheckArguments <- function(file_arbre, file_etude, file_compile, horizon, dec_perturb, dec_tbe1, tbe1, dec_tbe2, tbe2, ht, vol) {
  ...
  # si on spécifie une décennie de tbe, on doit aussi spécifier l'indice de tbe
    ...
    erreur <- c("Si dec_tbe1 est specifie, tbe1 doit etre specifié aussi")
  }
  # si on spécifie un indice de tbe, on doit aussi spécifier la décennie de tbe
  else if (dec_tbe1==0 & tbe1>0) {
    erreur <- c("Si tbe1 est specifie, dec_tbe1 doit etre specifié aussi")
  }
  ...
  # pour spécifier une 2e tbe, il faut en avoir spécifié une premiere
    ...
    erreur <- c("dec_tbe1 et tbe1 doivent etre specifiés pour pouvoir utiliser dec_tbe2")
  }
  ...
  # si on spécifie une décennie de tbe, on doit aussi spécifier l'indice de tbe
    ...
    erreur <- c("Si dec_tbe2 est specifié, tbe2 doit etre specifié aussi")
  }
  # si on spécifie un indice de tbe, on doit aussi spécifier la décennie de tbe
  else if (dec_tbe2==0 & tbe2>0) {
    erreur <- c("Si tbe2 est specifié, dec_tbe2 doit etre specifié aussi")
  }
  # l'indice de tbe doit être un entier 1-2-3-4-5
